...
class PPOAgent:
    def __init__(self, state_size, action_size, hidden_size=128):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=0.00005)  # 进一步降低学习率
        self.clip_ratio = 0.2
        self.value_coef = 0.5
        ...
        self.regularization = 0.05  # 增加正则化系数
        self.early_stopping = EarlyStopping(patience=10, min_delta=0.001)  # 增加耐心和降低最小变化量
    ...
    def update(self, batch_size=64):
        ...
        for _ in range(10):  # 多次更新
            ...
            for idx in range(0, len(states), batch_size):
                ...
                # 需要确保形状一致
                value_loss = self.value_coef * self.criterion(new_values.squeeze(), rewards[idx:idx + batch_size])
        ...
        # 更新学习率调度器
        self.scheduler.step(loss)  # 根据损失更新学习率
...
class RewardCalculator:
    ...
    def calculate_reward(self, current_state, previous_state, previous_action):
        ...
        reward -= self.weights['maj_faults'] * maj_faults * 0.5  # 减少惩罚权重
        ...
        reward += self.weights['avail_ratio'] * avail_ratio * 1.5  # 增加奖励权重
        ...
        if avail_ratio < 0.05:  # 可用内存<5%
            ...
            # 页错误惩罚加倍
            reward -= self.weights['maj_faults'] * maj_faults * 2 # 已经在前面乘以了基础权重，这里不再乘以2，而是直接加大权重
        else:
            self.emergency_mode = False
        ...
        if swap_free_ratio < 0.1:
            # 交换使用惩罚加倍
            reward -= self.weights['swap_ratio'] * swap_ratio * 2 # 已经在前面乘以了基础权重，这里不再乘以2，而是直接加大权重
